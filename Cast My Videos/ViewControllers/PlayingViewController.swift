...
class PlayingViewController: UIViewController, PlayerViewDelegate, UIPopoverPresentationControllerDelegate, DevicePopoverViewControllerDelegate
{
    ...
    {
        let durationInSeconds = CMTimeGetSeconds(duration!)
        currentTime = CMTimeMakeWithSeconds(
            Float64(scrubber.value) * durationInSeconds,
            Int32(currentTime.timescale)
        )
        updateTimerLabel()
    }
    ...
    func scrubberTouchUpInside(scrubber: UISlider!)
    {
        ...
        playerView.seekToTime(currentTime,
            completionHandler:
            {
                [weak self](done: Bool) -> Void in
    ...
    func devicePopoverViewControllerDidSelectDefaultDevice(
        viewController: DevicePopoverViewController
        )
    {
        playerView.allowAirplay = false
    }
    
    func devicePopoverViewControllerDidSelectAirPlay(
        viewController: DevicePopoverViewController!
        )
    {
        playerView.allowAirplay = true
