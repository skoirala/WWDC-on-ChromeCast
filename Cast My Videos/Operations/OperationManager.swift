...
class OperationManager: NSObject
{
        ...
        ) -> (firstOperation:NSOperation!, lastOperation: NSOperation!)
    {
        ...
        parseOperationProgress.addObserver(
            self,
            forKeyPath: "fractionCompleted",
            options: .New,
            context: ParseOperationObservingContext)

        let saveOperationProgress = saveOperation.progress
        saveOperation.addObserver(
            self,
            forKeyPath: "fractionCompleted",
            options: .New,
            context: SaveOperationObservingContext)
        return (networkOperation, saveOperation)
    }

    func fetchFromUrl()
    {
        var firstSetOfOperations = createOperationReturningFirstAndLastOperation(
            url: "https://developer.apple.com/videos/wwdc/2014/",
            forYear: "2014"
        )

        var secondSetOfOperations = createOperationReturningFirstAndLastOperation(
            url: "https://developer.apple.com/videos/wwdc/2013/",
            forYear: "2013"
        )

        secondSetOfOperations.firstOperation.addDependency(
            firstSetOfOperations.lastOperation
        )
    }
